...
describe("filters", function(){
	describe("filter functions", function(){
		it("Should have functions: request", function(){
			assert.equal(typeof filter.request, 'function');
		});
	});
	describe("filters implementation", function(){
		it("Should have response = {status: 400, message: 'No payloads'}", function(done){
			var filter = '';
	        chai.request(server)
            	.post('/')
            	.send({filter: filter})
		...
		it("Should have correct = {status: 200}", function(done){
			var payload = [
			        {
			            "address": {
			                "buildingNumber": "28",
